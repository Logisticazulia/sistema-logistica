document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function buscarMoto() {
    ...
    try {
      ...
      mostrarAlerta('success', '✅ Moto encontrada. Complete motivo y firmas.');
  ...
  function updatePreview() {
    ...
    document.getElementById('pv_ca_del').textContent = document.getElementById('caucho_del')?.value || '-';
    document.getElementById('pv_ca_tra').textContent = document.getElementById('caucho_tra')?.value || '-';
  ...
  inspectionForm?.addEventListener('submit', async e => {
    ...
    try {
      const payload = {
        ...
        motivo: document.getElementById('motivo_inspeccion').value, placa: document.getElementById('placa').value,
        ...
        coord_nombre: document.getElementById('coord_nombre').value, coord_cedula: document.getElementById('coord_cedula').value,
        insp_nombre: document.getElementById('insp_nombre').value, insp_cedula: document.getElementById('insp_cedula').value,
        inspector: usuarioActual.email || 'sistema', created_at: new Date().toISOString()
        // 🔜 Aquí se agregarán los componentes de motos cuando me pases la lista
      };
      const { error } = await supabase.from('inspecciones_pvr').insert([payload]);
      if(error) throw error;
      mostrarAlerta('success', '✅ Inspección de Moto registrada');
      setTimeout(() => { window.scrollTo({ top:0, behavior:'smooth' }); limpiarFormulario(); }, 1500);
