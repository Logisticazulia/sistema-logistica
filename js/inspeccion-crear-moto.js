document.addEventListener('DOMContentLoaded', async () => {
  ...
  // ==================== 🔵 BÚSQUEDA DE MOTOS Y ESPECIALES ====================
  async function buscarMoto() {
    ...
    try {
      ...
      if(!data) {
        mostrarAlerta('error', '❌ Moto no encontrada');
        toggleFormState(false);
        return;
      }
      
      // Verificar que sea moto o especial
      const esMoto = (data.tipo || '').toLowerCase().includes('moto') || 
                     (data.clase || '').toLowerCase().includes('moto') ||
                     (data.clase || '').toLowerCase() === 'especial';
      
      if(!esMoto) {
        mostrarAlerta('error', '⚠️ El vehículo encontrado no está registrado como Moto o Especial.');
        toggleFormState(false);
        return;
      }
