document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 🔍 FUNCIÓN DE BÚSQUEDA ACTUALIZADA (AHORA BUSCA POR S/CARROCERIA Y OTROS CAMPOS)
    async function buscarInspeccion() {
        const rawQuery = searchInput?.value.trim();
        if (!rawQuery) { mostrarAlerta('info', 'Ingrese N° Inspección, Placa o Serial para buscar'); return; }
        
        if (btnSearch) { btnSearch.disabled = true; if(btnSearchText) btnSearchText.style.display = 'none'; if(btnSearchLoader) btnSearchLoader.style.display = 'inline'; }
        mostrarAlerta('info', '🔍 Buscando inspección...');

        try {
            const q = rawQuery.replace(/\s+/g, '').toUpperCase();
            
            // ✅ AHORA BUSCA EN 5 CAMPOS DIFERENTES
            const { data, error } = await supabase.from('inspecciones_pvr').select('*')
                .or(`n_inspeccion.ilike.${q},placa.ilike.${q},s_carroceria.ilike.${q},n_identificacion.ilike.${q},s_motor.ilike.${q}`)
                .limit(1).maybeSingle();

            if (error) throw error;
            if (!data) { mostrarAlerta('error', '❌ Inspección no encontrada con: ' + rawQuery); toggleFormState(false); return; }
            ...
            if (btnSearch) { btnSearch.disabled = false; if(btnSearchText) btnSearchText.style.display = 'inline'; if(btnSearchLoader) btnSearchLoader.style.display = 'none'; }
        }
    }

    function limpiarFormulario() {
        if(searchInput) searchInput.value = ''; if(recordIdInput) recordIdInput.value = ''; toggleFormState(false);
        inspectionForm.reset(); updatePreview();
        mostrarAlerta('info', 'Ingrese N° Inspección, Placa o Serial para buscar');
    }
    ...
    inspectionForm?.addEventListener('submit', async (e) => {
        ...
        const btnText = btnSubmit?.querySelector('.btn-text'); const btnLoader = btnSubmit?.querySelector('.btn-loader');
        if(btnText) btnText.style.display = 'none'; if(btnLoader) btnLoader.style.display = 'inline';
        ...
        try {
            ...
            if(btnText) btnText.style.display = 'inline'; if(btnLoader) btnLoader.style.display = 'none';
        }
    });
