...
async function verificarDuplicadoEnTiempoReal(campo, valor, nombreCampo) {
    ...
    try {
        // ✅ BUSCAR EN TABLA vehiculos CON COINCIDENCIA EXACTA
        var result = await supabaseClient
            .from('vehiculos')
            .select('id, placa, facsimil, s_carroceria, s_motor, n_identificacion')
            .or('placa.eq.' + valor + ',facsimil.eq.' + valor + ',s_carroceria.eq.' + valor + ',s_motor.eq.' + valor + ',n_identificacion.eq.' + valor)
            .limit(1);
...
async function buscarVehiculo() {
    ...
    try {
        ...
        if (!data || data.length === 0) {
            mostrarAlerta('❌ No se encontró ningún vehículo con búsqueda EXACTA para: ' + searchTerm + '\n💡 Verifique que los datos coincidan exactamente', 'error');
            return;
        }
        ...
        // ✅ VERIFICAR SI YA EXISTE FICHA TÉCNICA PARA ESTE VEHÍCULO
