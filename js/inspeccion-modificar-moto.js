...
async function buscarInspeccion() {
    ...
    if(!q) { mostrarAlerta('info', '⚠️ Ingrese Placa, Serial, Cédula o Facsímil'); return; }
    ...
    try {
        ...
        // 🔍 Cláusula OR con los 6 campos solicitados
        // ⚠️ Si la columna 'facsimil' NO existe en Supabase, elimínala de esta línea.
        ...
        // 🏍️ FILTRO ESTRICTO: SOLO MOTOS
        // Nota: Si en tu BD el campo 'tipo' usa 'MOTOCICLETA', cambia '%MOTO%' por '%MOTOCICLETA%'
        ...
        if(!data) {
            mostrarAlerta('error', '❌ Inspección de MOTO no encontrada. Verifique el dato o que el vehículo sea tipo MOTO.');
        ...
        updatePreview();
        mostrarAlerta('success', '✅ Inspección de MOTO cargada. Edite y presione "Actualizar".');

    } catch(err) {
        console.error('❌ Error en búsqueda:', err);
        mostrarAlerta('error', `Fallo al buscar: ${err.message || 'Revise la consola F12'}`);
    } finally {
        if(btnSearch) {
    ...
    motoForm.addEventListener('submit', async e => {
        ...
        try {
            const payload = {
                ...
                motivo_inspeccion: document.getElementById('motivo_inspeccion')?.value || null,
