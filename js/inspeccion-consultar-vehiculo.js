document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 🆕 CARGA DE DATOS SEGURA (Sin usar JOIN complejo)
    async function cargarTodasInspecciones(page = 1) {
        try {
            resultsCount.textContent = '🔄 Cargando...';
            
            // 1. Obtener IDs de vehículos que SÍ son patrullas
            const { data: vehiculos, error: errVehiculos } = await supabase
                .from('vehiculos')
                .select('id')
            ...
            const idsPatullas = vehiculos ? vehiculos.map(v => v.id) : [];
            
            // Si no hay patrullas, mostrar vacío
            if (idsPatullas.length === 0) {
                renderTabla([]);
                return;
            }

            // 2. Obtener inspecciones filtradas por esos IDs
            const { data: inspecciones, error: errInsp, count } = await supabase
                .from('inspecciones_pvr')
                .select('id, n_inspeccion, fecha_inspeccion, hora, placa, s_carroceria, motivo, vehiculo_id', { count: 'exact' })
                .in('vehiculo_id', idsPatullas)
                .order('fecha_inspeccion', { ascending: false });

            if (errInsp) throw errInsp;

            allInspections = inspecciones || [];
    ...
    function renderTabla(dataToRender = null) {
        const data = dataToRender !== null ? dataToRender : filteredInspections;
    ...
    async function buscarVehiculo() {
        ...
        try {
            ...
            // ✅ Mismo truco: Buscar vehículo primero para validar clase y obtener ID
            const { data: vehiculo, error: errVeh } = await supabase.from('vehiculos').select('id, clase')
            ...
            // ✅ Verificar si es una patrulla permitida
            const claseVehiculo = (vehiculo.clase || '').trim().toUpperCase();
            if (!CLASES_PATRULLA.includes(claseVehiculo)) {
                mostrarAlerta('error', `❌ El vehículo encontrado es de clase "${claseVehiculo}" y no se permite en esta consulta.`);
                return;
            }

            // ✅ Buscar inspecciones de ESE vehículo específico
            ...
            if (filteredInspections.length === 0) {
                resultsSection.classList.remove('active');
                emptyState.style.display = 'block';
                emptyState.innerHTML = '<div class="icon">📭</div><p>Esta patrulla no tiene inspecciones registradas</p>';
            } else {
                resultsSection.classList.add('active');
                emptyState.style.display = 'none';
                renderTabla();
