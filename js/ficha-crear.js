...
function llenarFormulario(vehiculo) {
    console.log('📋 Llenando formulario con vehículo:', vehiculo.placa || vehiculo.id);
    console.log('🔍 Tipo en BD:', vehiculo.tipo);
    console.log('🔍 Clase en BD:', vehiculo.clase);
    ...
    Object.keys(camposNormales).forEach(function(dbField) {
        ...
        if (element && vehiculo[dbField]) {
            ...
            console.log('✅ Campo llenado:', dbField, '→', formField);
        }
    });
    
    // === ✅ CAMPO TIPO (SELECT) - CON MAPEO INTELIGENTE ===
    var tipoSelect = document.getElementById('tipo');
    if (tipoSelect) {
        var valorTipo = (vehiculo.tipo || '').toUpperCase().trim();
        var valorClase = (vehiculo.clase || '').toUpperCase().trim();
        
        console.log('🔧 Buscando coincidencia para tipo:', valorTipo, 'clase:', valorClase);
        
        // === MAPEO DE VALORES DE BD A OPCIONES DEL SELECT ===
        var mapeoTipo = {
            // Motos
            'MOTO': 'MOTO',
            'ENDURO': 'MOTO',
        ...
        // 1. Intentar coincidir con el mapeo
        var valorMapeado = mapeoTipo[valorTipo] || mapeoTipo[valorClase];
        
        if (valorMapeado) {
            // Buscar la opción en el select
            for (var i = 0; i < tipoSelect.options.length; i++) {
                if (tipoSelect.options[i].value === valorMapeado) {
                    tipoSelect.value = valorMapeado;
                    console.log('✅ Tipo encontrado (mapeo):', valorMapeado);
                    break;
                }
            }
        }
        
        // 2. Si no encontró con mapeo, intentar coincidencia directa
        if (!tipoSelect.value) {
            for (var j = 0; j < tipoSelect.options.length; j++) {
                var opt = tipoSelect.options[j];
                if (opt.value === valorTipo || opt.value === valorClase) {
                    tipoSelect.value = opt.value;
                    console.log('✅ Tipo encontrado (directo):', opt.value);
                    break;
                }
            }
        }
        
        // 3. Si aún no encontró, intentar coincidencia parcial
        if (!tipoSelect.value) {
            var busqueda = valorTipo || valorClase;
            for (var k = 0; k < tipoSelect.options.length; k++) {
                var opt2 = tipoSelect.options[k];
                if (busqueda.includes(opt2.value) || opt2.value.includes(busqueda)) {
                    tipoSelect.value = opt2.value;
                    console.log('✅ Tipo encontrado (parcial):', opt2.value);
                    break;
                }
            }
        }
        
        // 4. Si aún no encontró, dejar el primero después de "Seleccione"
        if (!tipoSelect.value && tipoSelect.options.length > 1) {
            console.log('⚠️ No se encontró coincidencia para tipo:', valorTipo);
        }
    }
    ...
    if (estatusSelect) {
        var valorEstatus = (vehiculo.estatus || vehiculo.situacion || '').toUpperCase().trim();
        
        console.log('🔧 Buscando estatus:', valorEstatus);
        
        // Normalizar valores
        var estatusMapeado = valorEstatus
            .replace('OPERATIVA', 'OPERATIVO')
            .replace('INOPERATIVA', 'INOPERATIVO')
            .replace('DESINCORPORADA', 'DESINCORPORADO');
        
        for (var m = 0; m < estatusSelect.options.length; m++) {
            var optStatus = estatusSelect.options[m];
            if (optStatus.value === estatusMapeado || optStatus.value === valorEstatus) {
    ...
    console.log('✅ Formulario llenado completado');
}
