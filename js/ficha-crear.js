 ...
 * - Cerrar sesión
 * - 9 gráficos con Chart.js (3 por fila)
...
function calculateStats(vehicles) {
    ...
    const motos = vehicles.filter(v => 
        ...
        (v.tipo && v.tipo.toUpperCase() === 'ENDURO')
    ).length;
    safeUpdate('totalMotos', motos);
    
    // Por estatus
    const operativos = vehicles.filter(v => 
        v.estatus && v.estatus.toUpperCase() === 'OPERATIVA'
    ).length;
    const inoperativos = vehicles.filter(v => 
        v.estatus && v.estatus.toUpperCase() === 'INOPERATIVA'
    ).length;
    const desincorporados = vehicles.filter(v => 
        v.estatus && v.estatus.toUpperCase() === 'DESINCORPORADA'
    ).length;
    ...
    safeUpdate('countCamioneta', vehicles.filter(v => 
        v.clase && v.clase.toUpperCase() === 'CAMIONETA'
    ).length);
    safeUpdate('countAutomovil', vehicles.filter(v => 
        v.clase && v.clase.toUpperCase() === 'AUTOMOVIL'
    ).length);
    safeUpdate('countCamion', vehicles.filter(v => 
        v.clase && v.clase.toUpperCase() === 'CAMION'
    ).length);
    
    // Por ubicación (CCPEM)
    safeUpdate('countCCPEM', vehicles.filter(v => 
        v.ubicacion_fisica && v.ubicacion_fisica.toUpperCase().includes('CCPEM')
    ).length);
...
/**
 * Renderiza todos los gráficos (9 en total)
...
function renderAllCharts(vehicles) {
    ...
    renderChartColor(vehicles);        // 9. Por Color ✅
}
...
function renderChartEstatus(vehicles) {
    const counts = {};
    vehicles.forEach(v => {
        const key = (v.estatus || 'SIN_DATO').toUpperCase();
...
function renderChartSituacion(vehicles) {
    const counts = {};
    vehicles.forEach(v => {
        const key = (v.situacion || v.estatus || 'SIN_DATO').toUpperCase();
        counts[key] = (counts[key] || 0) + 1;
    });
