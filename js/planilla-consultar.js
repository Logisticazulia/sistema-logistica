...
function setupSearchEnter() {
  if (searchInput) searchInput.addEventListener('keypress', e => { if (e.key === 'Enter') buscarPorPlacaFacsímil(); });
}
...
function exportarExcel() {
  ...
  const datosCompletos = filteredVehicles.map(v => ({
    id: v.id||'', marca: v.marca||'', modelo: v.modelo||'', tipo: v.tipo||'', clase: v.clase||'',
    ano: v.ano||'', color: v.color||'', s_carroceria: v.s_carroceria||'', s_motor: v.s_motor||'',
    placa: v.placa||'', facsimil: v.facsimil||'', n_identificacion: v.n_identificacion||'',
    situacion: v.situacion||'', unidad_administrativa: v.unidad_administrativa||'', redip: v.redip||'',
    ccpe: v.ccpe||'', epm: v.epm||'', epp: v.epp||'', ubicacion_fisica: v.ubicacion_fisica||'',
