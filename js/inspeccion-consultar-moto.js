...
async function buscarMoto() {
    ...
    try {
        ...
        // 1️⃣ Buscar en tabla vehiculos, pero SOLO si clase contiene MOTO
        ...
        if (!resultados || resultados.length === 0) {
            mostrarAlerta('error', '❌ No se encontró una MOTO con ese dato. El sistema busca exclusivamente motocicleta');
            return;
        }

        // 2️⃣ Buscar inspecciones PVR SOLO de ese vehículo válido
        const motoId = resultados[0].id;
        const { data, error: inspError } = await supabase.from('inspecciones_pvr')
            .select('id, n_inspeccion, fecha_inspeccion, hora, placa, s_carroceria, motivo')
            .eq('vehiculo_id', motoId)
            .or('tipo.ilike.%MOTO%,tipo.ilike.%ENDURO%,tipo.ilike.%PASEO%')
        ...
        if (filteredInspections.length === 0) {
            resultsSection.classList.remove('active');
            emptyState.style.display = 'block';
            emptyState.innerHTML = '<div class="icon">📭</div><p>Esta moto no tiene inspecciones PVR registradas</p>';
        } else {
            resultsSection.classList.add('active');
            emptyState.style.display = 'none';
            renderTabla();
        ...
        mostrarAlerta('success', `✅ Moto encontrada. Mostrando ${filteredInspections.length} inspección(es).`);
    } catch (err) {
...
// 🏍️ POBLAR VISTA PREVIA (ADAPTADO A JSONB DE MOTOS)
function poblarVistaPrevia(data) {
    ...
    set('pv_ano_tipo', `${data.ano || '-'} - ${data.tipo || 'MOTO'}`);
    ...
    // 🏍️ Cargar componentes desde JSONB (formato motos)
    ...
    if (compKeys.length > 0) {
        ...
        compGrid.innerHTML = '<div style="grid-column: 1/-1; text-align:center; color:#999; padding:20px;">Sin componentes registrados.</div>';
    }
}
