document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function cargarTodasInspecciones(page = 1) {
        try {
            ...
            // Filtro estricto para Motos
            const motoFilter = 'tipo.ilike.%MOTO%,tipo.ilike.%ENDURO%,tipo.ilike.%PASEO%';
            ...
            const { data, error } = await supabase
                .from('inspecciones_pvr')
                .select('id, n_inspeccion, fecha_inspeccion, hora, placa, s_motor, motivo')
                .or(motoFilter)
    ...
    function updatePaginationControls(totalItems) {
        ...
        resultsCount.textContent = `${totalItems} registro${totalItems !== 1 ? 's' : ''} encontrados`;
    }
    ...
    async function buscarMoto() {
        ...
        if (!rawQuery) {
            mostrarAlerta('info', '📝 Ingrese Placa, Serial o Identificación para buscar.');
            return;
        }
        ...
        try {
            ...
            // Búsqueda directa en inspecciones (más rápido y preciso)
            const { data, error } = await supabase
                .from('inspecciones_pvr')
                .select('id, n_inspeccion, fecha_inspeccion, hora, placa, s_motor, motivo')
                .or(`placa.ilike.${q},s_motor.ilike.${q},n_identificacion.ilike.${q}`)
                .or(motoFilter)
                .order('fecha_inspeccion', { ascending: false });
            ...
            if (filteredInspections.length === 0) {
                mostrarAlerta('error', '❌ No se encontraron inspecciones para esa motocicleta.');
                resultsSection.classList.remove('active');
                emptyState.style.display = 'block';
            } else {
                renderTabla();
    ...
    document.addEventListener('keydown', (e) => { 
        if (e.key === 'Escape' && detailModal && detailModal.classList.contains('active')) cerrarModal(); 
    });

    if (btnPrev) btnPrev.addEventListener('click', () => { 
        if (currentPage > 1) { currentPage--; renderTabla(); updatePaginationControls(filteredInspections.length || allInspections.length); resultsSection.scrollIntoView({ behavior: 'smooth' }); } 
    });
    if (btnNext) btnNext.addEventListener('click', () => { 
        const tp = Math.ceil((filteredInspections.length || allInspections.length) / ITEMS_PER_PAGE); 
        if (currentPage < tp) { currentPage++; renderTabla(); updatePaginationControls(filteredInspections.length || allInspections.length); resultsSection.scrollIntoView({ behavior: 'smooth' }); } 
    });
