document.addEventListener('DOMContentLoaded', async () => {
  ...
  function renderTabla() {
    ...
    pageData.forEach(insp => {
      ...
      tr.innerHTML = `
        ...
        <td class="s-motor">${insp.s_motor || '-'}</td>
  ...
  async function buscarMoto() {
    ...
    if(!q) { mostrarAlerta('info', 'Ingrese N° Inspección o Placa para buscar'); return; }
    ...
    try {
      ...
      const { data, error } = await supabase.from('inspecciones_pvr')
        .select('id, n_inspeccion, fecha_inspeccion, hora, placa, s_motor, motivo')
        .or(`n_inspeccion.ilike.${cleanQ},placa.ilike.${cleanQ},s_motor.ilike.${cleanQ}`)
      ...
      if (filteredInspections.length === 0) {
        resultsSection.classList.remove('active');
        emptyState.style.display = 'block';
        emptyState.innerHTML = '<div style="font-size: 3rem; margin-bottom: 15px; opacity: 0.5;">📭</div><p>No se encontraron inspecciones con ese criterio</p>';
      } else {
        resultsSection.classList.add('active');
        emptyState.style.display = 'none';
        renderTabla();
      ...
      mostrarAlerta('success', `✅ Búsqueda completada. ${filteredInspections.length} resultado(s).`);
  ...
  async function abrirDetalle(id) {
    try {
      ...
      // 🆕 Renderizar componentes desde JSONB dinámicamente
