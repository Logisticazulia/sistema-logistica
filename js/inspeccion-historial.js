document.addEventListener('DOMContentLoaded', async () => {
    ...
    const esMoto = (row) => {
        ...
        if (t.includes('moto') || t.includes('enduro') || t.includes('paseo') || t.includes('trimovil') || t.includes('traccion')) return true;
        // 3. Fallback por modelo/marca
        const m = (row.modelo || '').toLowerCase();
        if (['dr ','dr650','dr-650','xt','ybr','xlr','bross','falcon','xcape','vstrom','kdx','klr','lechuza','brf','xcape','owen','tx'].some(k => m.includes(k))) return true;
        return false;
    };

    // 🔹 HELPER: OBTENER COMPONENTES (JSONB MOTO + COLUMNAS PATRULLA)
    const obtenerComponentes = (row) => {
        const comps = {};
        if (row.componentes_moto) {
            let c = row.componentes_moto;
            if (typeof c === 'string') { try { c = JSON.parse(c); } catch(e){ c={}; } }
            Object.assign(comps, c);
        }
        const ignoreKeys = ['id','vehiculo_id','n_inspeccion','fecha_inspeccion','hora','motivo','lugar','asignacion','supervision','placa','marca','modelo','ano','tipo','color','n_identificacion','s_carroceria','kms','inspector','created_at','observaciones','coord_nombre','coord_rango','coord_cedula','coord_telefono','insp_nombre','insp_rango','insp_cedula','insp_telefono','componentes_moto','s_motor','facsimil'];
        Object.keys(row).forEach(k => {
            if (!ignoreKeys.includes(k)) {
                const val = (row[k] || '').toString().toUpperCase();
                if (['B', 'M', 'NT'].includes(val)) comps[k] = val;
            }
        });
        return comps;
    };
